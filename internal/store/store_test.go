...
func TestVerifyRejectsExtraJSONValueOnJournalLine(t *testing.T) {
	cfg := storeConfig(t)
	store, err := Open(cfg)
	if err != nil {
		t.Fatal(err)
	...
	if len(data) == 0 || data[len(data)-1] != '\n' {
		t.Fatal("generated journal is not newline terminated")
	}
	...
	if _, err := Verify(cfg.JournalPath()); err == nil {
		t.Fatal("expected verifier to reject a second JSON value on one line")
	}
}
